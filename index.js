...
for (let i = 0; i < files.length; i++) {
  ...
  const base64Image = fs.readFileSync("public/" + files[i], {
  encoding: "base64",
  });
    ...
    input: [
      {
        role: "user",
        content: [
          { type: "input_text", text: "Extraia TODO o texto que esteja sublinhado em verde e retorne APENAS o texto puro. NÃO alterar o texto original." },
          {
            type: "input_image",
  ...
  let fileName = `WhatsMyNote(file ${i}) - ${dateNow.getFullYear()}${(dateNow.getMonth()+1) < 10 ? '0'+(dateNow.getMonth()+1) : (dateNow.getMonth()+1)}${dateNow.getDate() < 10 ? '0' + dateNow.getDate() : dateNow.getDate()}${dateNow.getHours()}${dateNow.getMinutes()}${dateNow.getSeconds()}.md`
